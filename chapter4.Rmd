...
K-means algorithm is one of the simplest clustering algorithm.  It partions n observations into k clusters. The clustering policy is that each observation belongs to the cluster with nearest mean. This is an unsupervised learning algorithm.
...
Now we perform the K-means clustering on the Boston dataset. We scale the dataset and calculate the distance matrix by finding the euclidean diatance between the variables. We determine the number of clusters by plotting the  the total of within cluster sum of squares (WCSS) behaves when the number of cluster changes.
```{r echo=TRUE}
# access the MASS package
library(MASS)
library(GGally)
...
p <- ggpairs(boston_scaled_df, aes(col=km$cluster), lower = list(combo = wrap("facethist", bins = 20))) + ggtitle("k-means clustering of Boston(scaled) dataset")
#ggpairs(Boston, col = km$cluster)

```

### 8. K- means on Boston data with clusters
...
lda.arrows(lda.fit.cluster, myscale = 1.5)
```
